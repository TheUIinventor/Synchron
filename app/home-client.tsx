...
export default function HomeClient() {
  const { timetable, isLoading, error, refreshTimetable } = useTimetable();
  ...
  // Helper to find periods (simplified logic for UI demo)
  const periods = timetable?.days?.[0]?.periods || [];
  const currentPeriod = periods[0]; // Placeholder logic
        ...
        <div className="md:col-span-8 space-y-4">
              ...
              <div className="relative z-10 flex flex-col h-full justify-between min-h-[200px]">
                ...
                <div className="mt-4">
                  <h2 className="text-4xl md:text-6xl font-serif leading-tight mb-2">
                    {currentPeriod?.name || "Free Period"}
                  </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <div className="rounded-m3-xl bg-surface-container-high p-6 flex flex-col justify-between hover:bg-surface-variant transition-colors duration-300">
                  ...
                  <div>
                    <h3 className="text-xl font-bold mb-1">{nextPeriod?.name || "Recess"}</h3>
                    <p className="text-muted-foreground">{nextPeriod?.room || "Quad"}</p>
                  </div>
               </div>

               <div className="rounded-m3-xl bg-tertiary-container text-tertiary-container-foreground p-6 flex flex-col justify-between hover:brightness-95 transition-all cursor-pointer">
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <p className="text-sm font-medium opacity-80">Unread</p>
                    </div>
                    ...
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="flex gap-4 items-center group cursor-pointer">
                            <div className="flex flex-col items-center min-w-[3rem]">
                                <span className="text-xs font-bold text-muted-foreground">1{i}:00</span>
                            </div>
                            <div className="flex-1 p-3 rounded-xl bg-surface hover:bg-white dark:hover:bg-zinc-800 border border-transparent hover:border-border transition-all shadow-sm">
