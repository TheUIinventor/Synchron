                        ...
                        <div className="flex flex-col items-center min-w-[2.5rem] sm:min-w-[3rem]">
                            ...
                            <a href={link} target="_blank" rel="noopener noreferrer" className={`${cardClass} flex items-stretch gap-2`}>
                              {(() => {
                                ...
                                return (
                                  <div className="flex items-center justify-between w-full rounded-md p-2" style={cardTint}>
                                    {displayColour ? (
                                      <div className="w-1 min-w-[4px] rounded-lg self-stretch mr-2" style={{ backgroundColor: `#${displayColour}` }} />
                                    ) : null}
                                    <div className="flex-1 flex items-center justify-between gap-3 min-w-0">
                                      <div className="flex items-center gap-3 min-w-0">
                                        <div className={`rounded-md px-3 py-1 text-sm font-medium truncate ${getSubjectColor(period.subject)}`} style={getSubjectColorStyle(period.subject)}>
                                          {period.subject}
                                        </div>
                                      </div>
                                      <div className="flex items-center gap-2 shrink-0 text-right">
                                        {isSubstitutePeriod(period) ? (
                                          <span className="inline-block px-2 py-0.5 rounded-md text-xs font-medium truncate max-w-[92px]" style={{ backgroundColor: 'hsl(var(--accent))', color: 'hsl(var(--accent-foreground))' }}>{displayTeacher(period)}</span>
                                        ) : (
                                          <span className="text-on-surface-variant truncate max-w-[92px]">{displayTeacher(period)}</span>
                                        )}
                                        <span className="text-on-surface-variant">•</span>
                                        <span className={`ml-2 text-sm font-medium px-2 py-0.5 rounded-md min-w-[44px] text-center`} style={displayColour ? { backgroundColor: `#${displayColour}22` } : {}}>{(period as any).displayRoom || period.room}</span>
                                      </div>
                                    </div>
                                  </div>
                                )
