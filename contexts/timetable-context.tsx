  ...
  const timetableData: Record<string, Period[]> = useMemo(() => {
    ...
    if (useExternalTimetableByWeek) {
      ...
      for (const [day, groups] of Object.entries(useExternalTimetableByWeek as Record<string, { A: Period[]; B: Period[]; unknown: Period[] }>)) {
        ...
        // REMOVED: Variation application in useMemo that was causing re-render loops
        // Variations are now handled entirely by the setExternalTimetable wrapper
        // which merges saved variations BEFORE setting state, ensuring they're never lost.
      }
